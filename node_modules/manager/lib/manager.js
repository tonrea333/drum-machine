...
var manageWorkers = function () {
    if (workers.length > 0) {
        ...
        var worker, workersCount = parseInt(build.workers) || 1;

        // Fork workers
        for (var i = 0; i < workersCount; i++) {
            worker = cluster.fork();
            worker.send({ config: build });
            workers.push(worker);
        }
    }
}

var assetWatcher = function (arr, type) {
    ...
    for (var id in arr) {
        (function (id, res) {
            ...
            checkSync(res, id, type, res.watch);
        })(id, arr[id]);
    }
}
...
module.exports.css = function (id) {
    ...
    if (!build.enable) {
        for (var i = 0; i < build.css[id].input.length; i++) {
            out += '<link href="' + build.css[id].input[i].replace('./media', '') + '" rel="stylesheet"/>\n';
        }
    } else {
        out = '<link href="' + build.css[id].url + '" rel="stylesheet"/>';
    }
...
module.exports.js = function (id) {
    ...
    if (!build.enable) {
        for (var i = 0; i < build.js[id].input.length; i++) {
            out += '<script src="' + build.js[id].input[i].replace('./media', '') + '" type="text/javascript"></script>\n';
        }
    } else {
        out = '<script src="' + build.js[id].url + '" type="text/javascript"></script>';
    }
...
module.exports.start = function (config) {
    ...
    if (cluster.isMaster) {
        ...
        sourceWatcher(); // Monitor source code modifications

        if (build.enable != false) {
